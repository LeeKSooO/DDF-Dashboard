...
const trafficKpiTooltips = {
  weekdayRide: "주중(월~금) 평균 승차 인원입니다.",
  weekdayAlight: "주중(월~금) 평균 하차 인원입니다.",
  weekendRide: "주말(토~일) 평균 승차 인원입니다.",
  weekendAlight: "주말(토~일) 평균 하차 인원입니다.",
  weekdayMorningPeak: "주중 아침 시간대의 최대 이용량입니다.",
  weekdayEveningPeak: "주중 저녁 시간대의 최대 이용량입니다.",
  weekendPeak: "주말 최대 이용량입니다.",
};

export const TrafficContent = memo(function TrafficContent({
  selectedMonth,
  selectedRegion,
}: TrafficContentProps) {
  ...
  const getTrafficStats = (data: TrafficResponse | null) => {
    ...
    // 24시간 총합 계산
    const totalWeekdayRide = data.weekday_patterns.reduce(
      (sum, pattern) => sum + pattern.avg_ride_passengers,
      0
    );
    const totalWeekdayAlight = data.weekday_patterns.reduce(
      (sum, pattern) => sum + pattern.avg_alight_passengers,
      0
    );
    const totalWeekendRide = data.weekend_patterns.reduce(
      (sum, pattern) => sum + pattern.avg_ride_passengers,
      0
    );
    const totalWeekendAlight = data.weekend_patterns.reduce(
      (sum, pattern) => sum + pattern.avg_alight_passengers,
      0
    );
    ...
    const loadCurrentData = async () => {
      try {
        ...
        if (selectedRegion === "전체") {
          ...
          response = await apiService.getHourlyTraffic(
            analysisMonth,
            "district",
            selectedRegion
          );
  ...
  function CurrentTrafficView() {
    ...
    // 24시간 승하차 차트 데이터 준비
    const chartData =
      currentData?.weekday_patterns.map((weekday, index) => ({
        hour: `${index.toString().padStart(2, "0")}:00`,
        weekday_boarding: Math.round(weekday.avg_ride_passengers * 100) / 100,
        weekday_alighting:
          Math.round(weekday.avg_alight_passengers * 100) / 100,
        weekend_boarding:
          Math.round(
            (currentData.weekend_patterns[index]?.avg_ride_passengers || 0) *
              100
          ) / 100,
        weekend_alighting:
          Math.round(
            (currentData.weekend_patterns[index]?.avg_alight_passengers || 0) *
              100
          ) / 100,
      })) || [];
          ...
          <Card className="bg-gray-50">
            <CardContent className="p-6">
              ...
              <div className="text-4xl font-bold text-gray-900 mt-4">
                ...
                {getTrafficStats(currentData)?.totalWeekdayRide && (
                  <span className="text-lg text-gray-600 ml-1">명</span>
                )}
              </div>
            </CardContent>
          </Card>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="text-6xl font-bold text-blue-600 mb-4">
                    {currentData
                      ? `${currentData.peak_hours.weekday_morning_peak.avg_total_passengers.toFixed(
                          2
                        )}명`
                      : "24.00명"}
                  </div>
                  <Badge
                    variant="secondary"
                    className="text-base px-3 py-1 bg-gray-300"
                  >
                    {currentData
                      ? `${currentData.peak_hours.weekday_morning_peak.hour}:00`
                      : "8:00"}
                  </Badge>
                </div>
                ...
                <div className="w-48 h-48">
                  {(() => {
                    const morningPeakHour =
                      currentData?.peak_hours.weekday_morning_peak.hour || 8;
                    const morningData =
                      currentData?.weekday_patterns[morningPeakHour];
                    const rideValue = morningData
                      ? morningData.avg_ride_passengers
                      : 12;
                    const alightValue = morningData
                      ? morningData.avg_alight_passengers
                      : 12;
                ...
                {(() => {
                  ...
                  if (morningData) {
                    const total =
                      ...
                      morningData.avg_alight_passengers;
                    const ridePercent = (
                      (morningData.avg_ride_passengers / total) *
                      100
                    ).toFixed(1);
                    const alightPercent = (
                      (morningData.avg_alight_passengers / total) *
                      100
                    ).toFixed(1);
                    return (
                      <>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="text-6xl font-bold text-red-600 mb-4">
                    {currentData
                      ? `${currentData.peak_hours.weekday_evening_peak.avg_total_passengers.toFixed(
                          2
                        )}명`
                      : "23.00명"}
                  </div>
                  ...
                  >
                    {currentData
                      ? `${currentData.peak_hours.weekday_evening_peak.hour}:00`
                      : "18:00"}
                  </Badge>
                </div>
                ...
                <div className="w-48 h-48">
                  {(() => {
                    const eveningPeakHour =
                      currentData?.peak_hours.weekday_evening_peak.hour || 18;
                    const eveningData =
                      currentData?.weekday_patterns[eveningPeakHour];
                    const rideValue = eveningData
                      ? eveningData.avg_ride_passengers
                      : 11;
                    const alightValue = eveningData
                      ? eveningData.avg_alight_passengers
                      : 12;
                ...
                {(() => {
                  ...
                  if (eveningData) {
                    const total =
                      ...
                      eveningData.avg_alight_passengers;
                    const ridePercent = (
                      (eveningData.avg_ride_passengers / total) *
                      100
                    ).toFixed(1);
                    const alightPercent = (
                      (eveningData.avg_alight_passengers / total) *
                      100
                    ).toFixed(1);
                    return (
                      <>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="text-6xl font-bold text-green-600 mb-4">
                    {currentData
                      ? `${currentData.peak_hours.weekend_peak.avg_total_passengers.toFixed(
                          2
                        )}명`
                      : "13.00명"}
                  </div>
                  ...
                  >
                    {currentData
                      ? `${currentData.peak_hours.weekend_peak.hour}:00`
                      : "17:00"}
                  </Badge>
                </div>
                ...
                <div className="w-48 h-48">
                  {(() => {
                    const weekendPeakHour =
                      currentData?.peak_hours.weekend_peak.hour || 17;
                    const weekendData =
                      currentData?.weekend_patterns[weekendPeakHour];
                    const rideValue = weekendData
                      ? weekendData.avg_ride_passengers
                      : 7;
                    const alightValue = weekendData
                      ? weekendData.avg_alight_passengers
                      : 6;
                ...
                {(() => {
                  ...
                  if (weekendData) {
                    const total =
                      ...
                      weekendData.avg_alight_passengers;
                    const ridePercent = (
                      (weekendData.avg_ride_passengers / total) *
                      100
                    ).toFixed(1);
                    const alightPercent = (
                      (weekendData.avg_alight_passengers / total) *
                      100
                    ).toFixed(1);
                    return (
                      <>
  ...
  function DistrictsTrafficView() {
    ...
    const [selectedDistricts, setSelectedDistricts] =
      useState<string[]>(getInitialDistricts);
    const [districtData, setDistrictData] = useState<
      Record<string, TrafficResponse>
    >({});
    const [loading, setLoading] = useState(false);
    const [patternType, setPatternType] = useState<"weekday" | "weekend">(
      "weekday"
    );
    ...
    const boardingChartData: ChartDataPoint[] = Array.from(
      { length: 24 },
      (_, hour) => {
    ...
    const alightingChartData: ChartDataPoint[] = Array.from(
      { length: 24 },
      (_, hour) => {
        const data: ChartDataPoint = {
          hour: `${hour.toString().padStart(2, "0")}:00`,
    ...
    // 구별 총합 데이터 (승차/하차 분리)
    const summaryData = selectedDistricts
      .map((district) => {
        ...
        const weekdayRide = data.weekday_patterns.reduce(
          (sum, pattern) => sum + pattern.avg_ride_passengers,
          0
        );
        const weekdayAlight = data.weekday_patterns.reduce(
          (sum, pattern) => sum + pattern.avg_alight_passengers,
          0
        );
        const weekendRide = data.weekend_patterns.reduce(
          (sum, pattern) => sum + pattern.avg_ride_passengers,
          0
        );
        const weekendAlight = data.weekend_patterns.reduce(
          (sum, pattern) => sum + pattern.avg_alight_passengers,
          0
        );
        ...
        <Card className="bg-gray-50">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              {selectedRegion && selectedRegion !== "전체" && (
                <div>
                  ...
                  <Badge
                    variant="default"
                    className="px-4 py-2 text-lg bg-blue-600"
                  >
                    <div
                      className="w-4 h-4 rounded-full mr-3"
                      style={{
                        backgroundColor: getDistrictColor(selectedRegion),
                      }}
                    ></div>
                    {selectedRegion}
            ...
            {(() => {
              ...
              const availableDistricts = utils.seoulDistricts.filter(
                (d) => !selectedDistricts.includes(d)
              );

              return (
                selectedDistricts.length < maxDistricts &&
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-gray-50">
                <CardHeader>
                  <CardTitle className="text-2xl font-bold">
                    {patternType === "weekday" ? "주중" : "주말"} 구별 승차 패턴
                  </CardTitle>
                      ...
                      <LineChart data={boardingChartData}>
                        ...
                        {selectedDistricts.map((district) => (
                          <Line
                            ...
                            dot={{
                              r: isHeaderSelectedDistrict(district) ? 3 : 2,
                            }}
                          />
                        ))}
                      </LineChart>
              ...
              <Card className="bg-gray-50">
                <CardHeader>
                  <CardTitle className="text-2xl font-bold">
                    {patternType === "weekday" ? "주중" : "주말"} 구별 하차 패턴
                  </CardTitle>
                  <CardDescription className="text-lg">
                    {patternType === "weekday"
                      ? "주중 (월~금)"
