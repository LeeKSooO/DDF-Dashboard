...
class HeatmapService:
    ...
    async def get_seoul_heatmap(
        ...
        try:
            ...
            # 1. 서울시 경계 조회 (더미 데이터로 시작)
            seoul_boundary = await self._get_seoul_boundary(db)
            
            # 2. 구별 교통량 집계 조회
            districts_data = await self._get_districts_traffic_data(
    ...
    async def _get_seoul_boundary(self, db: AsyncSession) -> BoundarySchema:
        """서울시 경계 좌표 조회"""
        try:
            # 서울시 전체 경계 조회 (외곽선 추출)
            boundary_query = text("""
